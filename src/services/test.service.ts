...
export async function insertNewTest(
	name: string,
	pdfUrl: string,
	category: string,
	teacher: string,
	...
	const checkdiscipline = await testRepository.findDisciplinesByName(
		discipline
	)
	if (!checkdiscipline) {
		throw {
	...
	const checkTeacherDiscipline = await testRepository.findteacherDisciplineId(
		checkTeacher.id,
		checkdiscipline.id
	)
	...
	const result = await testRepository.insert(
		name,
		pdfUrl,
		checkCategory.id,
		checkTeacherDiscipline[0].id
	)
	return result
}
...
export async function getTestsByDiscipline() {
	...
	if (!result) {
		throw {
			status: 400,
			message: `Erro on get tests by discipline`,
		}
	}
	return result
}
...
export async function getTestsByTeacher() {
	const result = await testRepository.findAllTestsGroupByTeacher()
	if (!result) {
		throw {
			status: 400,
			message: `Erro on get tests by teacher`,
		}
	}
	return result
}
